/*
Map Class
Accepts raw data from a file, removes whitespace, punctuation, and
capitalization, and tokenizes the data into distinct words. Periodically outputs
...
bool isPunctOrSpace(int aChar) { return (ispunct(aChar) || isspace(aChar)); }

// tokenizes the value into distinct words, removing whitespace, punctuation,
// and capitalization
void Map::map(const string& aKey, const string& aValue) {
  string tempValue = aValue;
  string outputBuffer;
  // loop through the value buffer, parsing words, buffering and exporting
  // output data
  while (tempValue.size() > 0) {
    ...
    if (*it == '\'') {
      ...
      if (*(it + 1) == 's') {
        // we found an s after the apostrophe, check if it is an "is"
        // contraction such as "it's"
        ...
        if (compareStr == "it" || compareStr == "he" || compareStr == "she" ||
            compareStr == "who" || compareStr == "what" ||
            compareStr == "when" || compareStr == "where" ||
            compareStr == "why" || compareStr == "how" ||
            compareStr == "which" || compareStr == "this" ||
            compareStr == "that" || compareStr == "here" ||
            compareStr == "there" || compareStr == "somebody" ||
        ...
        // we found "n't", this is a "not" contraction, check up to foundIdx - 1
        // so the compare string doesn't have the n
        ...
        // compare to n't contractions that aren't just the word in front of the
        // n't
        if (compareStr == "ai") {
        ...
        // this didn't fit any contraction logic, so it's a weird word with an
        // apostrophe find the next punctuation or space so we keep the
        // apostrophe
    ...
    if (foundIdx == 0) {
      ...
      outputBuffer += "(\"";
      // loop through the sub string, adding the word as a lowercase string to
      // the output buffer
      for (char c : subStr) {
      ...
      if (tempValue.size() > 0) {
        // erase the whitespace or punctuation, and trailing s if necessary,
        // then continue
        tempValue.erase(0, 1);
      }
    }
    ...
    // if the output buffer has over 100 characters in it, export the data to
    // the temporary file
  ...
  // ensure we export any remaining output if we finish parsing the data and the
  // output buffer isn't empty
...
int main() {
  ...
  // map.map("file", "you're we're they're who're what're when're where're
  // why're how're which're"); Map mapguy; string testStr = "This here is a
  // string's string, go ahead and buffer this. You're probably going to see
  // who's got apostrophes"; string fileName = "file"; mapguy.map(fileName,
  // testStr); string allswell = "ACT I\n"; allswell += "SCENE I. Rousillon. The
  // COUNT's palace.\n\n"; allswell += "Enter BERTRAM, the COUNTESS of
  // Rousillon, HELENA, and LAFEU, all in black\n"; allswell += "COUNTESS\n";
